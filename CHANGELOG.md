...
- Home leaderboard/activity queries soft-fail so the marketing shell still renders
- Local/VPS agent worker backs off 15 minutes on 402 quota instead of heartbeat-spinning
...
- Marketing home keeps a two-link top bar plus command palette. Signed-in account is a menu, not a wrapping chip row.
- Project banners stay full-frame: contain (no crop) on cards, ships, and upload preview. Hero opens a full-size lightbox.
- Dashboard header drops duplicate Tasks / Propose / Sign out (those live in the rail and account menu).

## 2026-08-13 - Dashboard interior + hub bump (v2.2.1)

- Dashboard: KPI strip, section jump nav, inbox/work/tools/profile grouping, tokenized chrome
- Relative timestamps; empty states with a next action
- Command palette: cockpit, ships, review queue, good-first, forge
- Login headline matches 4.6 hero; cockpit KPIs use display type
- Hub Paths + hive + EN FAQ bumped to "ships in public"; hive still refreshed
- Ensure-TweetCard.ps1: curl line-array no longer crashes on $Matches

## 2026-08-13 - Grok Build 4.6 polish (v2.2.0)

- Fontshare Clash Display + Satoshi on marketing and chrome (Geist remains fallback)
- Semantic motion/surface tokens so all 11 themes actually recolor header, cards, CTAs
- Benefit-first home hero, one primary CTA, signed-in flywheel no longer buries the fold
- Card blurbs strip BUILDER TL;DR walls (`publicProjectBlurb`)
- Home FAQ + FAQPage JSON-LD, llms.txt alternate, Twitterbot robots allow
- OG / tweet card cache-bust `og.jpg?v=2.2.0`
