...
Primary auth: **Sign in with X** (OAuth 2.0). Signed-in users never see Sign in CTAs - they get Propose / Tasks / Dashboard instead.
...
- **Seal & Ship (Strike the Anvil)** - when all leaves are accepted, the creator seals a versioned ZIP package + permanent public ship page (`/projects/{slug}/seal`, `/projects/{slug}/ship`, download `/api/projects/{slug}/package`)
...
- **Nightcap gift** - report leftover token capacity to platform or a project (no API keys stored)
