...
export default async function ShipPage({
  ...
  const project = await prisma.project.findUnique({
    where: { slug },
    include: {
      proposer: { select: { handle: true, name: true } },
      tasks: {
  ...
  const primary =
    project.artifacts.find((a) => a.isPrimary) || project.artifacts[0];
  if (!primary) {
    return (
  ...
  const contributors = new Map<string, { handle: string | null; name: string | null }>();
  for (const t of project.tasks) {
    for (const c of t.contributions) {
      contributors.set(c.user.id, { handle: c.user.handle, name: c.user.name });
    }
  }
      ...
      <section className="space-y-3">
        ...
        <ul className="flex flex-wrap gap-2">
          {[...contributors.values()].map((c) => (
            <li key={c.handle || c.name || Math.random()}>
              {c.handle ? (
                <Link
                  href={`/u/${c.handle}`}
