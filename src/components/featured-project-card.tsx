...
export function FeaturedProjectCard({
  ...
  return (
    <aside
      className={`relative flex h-auto w-full max-w-full flex-col overflow-hidden rounded-2xl border bg-gradient-to-br p-3.5 shadow-[0_0_48px_rgba(245,158,11,0.12)] sm:min-h-[14rem] sm:justify-between sm:p-5 lg:h-full lg:min-h-[16rem] ${
        done || sealed
          ? "border-emerald-500/40 from-emerald-500/15 via-[#050505]/95 to-[#050505]"
          : "border-amber-500/35 from-amber-500/10 via-[#050505]/95 to-[#050505]"
      }`}
      aria-label="Featured project"
    >
      <div className="min-w-0">
        {/* Banner can dominate narrow screens; keep it short on mobile */}
        <div className="max-h-28 overflow-hidden sm:max-h-none">
          <ProjectBannerThumb url={project.bannerUrl} title={project.title} />
        </div>
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span className="rounded-full border border-amber-500/50 bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-amber-200">
            Featured
          </span>
        ...
        <Link
          ...
          className="mt-2 block text-lg font-bold leading-snug text-white hover:text-amber-200 sm:mt-3 sm:text-2xl"
        >
          {project.title}
        </Link>
        <p className="mt-1.5 line-clamp-2 text-sm text-stone-400 sm:mt-2 sm:line-clamp-3">
          {project.description}
        </p>
