...
export function MatchingFundsPanel({
  ...
  if (!canEdit && !matchingEnabled && matchingPoolCents < 1) {
    return null;
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xs text-stone-400">
            Transparent match pool: community donations get extra capital from this
            budget at {ratioLabel(matchingRatioBps)}. Match spends are public ledger
            events.
          </p>
        </div>
        {matchingEnabled && matchingRemainingCents > 0 ? (
          ...
          <span className="rounded-full border border-white/10 px-2.5 py-0.5 text-[11px] text-stone-500">
            {matchingEnabled ? "Pool empty" : "Off"}
          </span>
        )}
      </div>
        ...
        <div className="space-y-3 border-t border-white/5 pt-3">
          ...
          <div className="flex flex-wrap items-end gap-2">
            <div className="min-w-[8rem] flex-1">
              <Label htmlFor="fundUsd">Add to match pool (USD)</Label>
              <Input
                id="fundUsd"
                type="number"
                min={1}
