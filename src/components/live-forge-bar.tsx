...
function CountUp({ value, className }: { value: number; className?: string }) {
  ...
  return (
    <span className={className ?? "tabular-nums font-bold text-[var(--accent)]"}>
      {n.toLocaleString()}
    </span>
  );
}
...
export function LiveForgeBar({ stats }: { stats: LiveForgeNumbers }) {
  ...
  return (
    <section
      className="relative z-0 overflow-hidden rounded-2xl border border-[color:var(--accent)]/40 bg-gradient-to-br from-[color:var(--accent)]/10 via-[var(--background)] to-[var(--background)] p-4 shadow-[0_0_48px_var(--glow)] sm:rounded-3xl sm:p-6"
      aria-label="Live Forge stats"
      aria-live="polite"
    >
      <div className="pointer-events-none absolute -right-8 top-0 h-32 w-32 rounded-full bg-[color:var(--accent)]/15 blur-3xl" />
      <div className="pointer-events-none absolute -left-6 bottom-0 h-24 w-24 rounded-full bg-[color:var(--bronze)]/10 blur-2xl" />
      ...
      <div className="relative mb-4 flex flex-wrap items-end justify-between gap-3 sm:mb-5">
        <div>
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <h2 className="font-display text-sm font-black uppercase tracking-[0.22em] text-[var(--accent)] sm:text-base">
              Live Forge
            </h2>
          ...
          <p className="mt-1.5 max-w-xl text-xs text-stone-500 sm:text-sm">
            Single network pulse - visitors, builders, active and completed projects, open
            tasks. Refreshes about every 30s while this page is open.
          </p>
        </div>
      </div>
          ...
          <div
            ...
            className={`rounded-2xl border px-3 py-4 sm:px-4 sm:py-5 ${
              it.tone === "emerald"
                ...
                : "border-[color:var(--accent)]/25 bg-black/45"
            }`}
          >
            ...
            <dd className="mt-2">
              <CountUp
                ...
                className={`block text-3xl font-black tabular-nums leading-none tracking-tight sm:text-4xl md:text-5xl ${
                  it.tone === "emerald" ? "text-emerald-300" : "text-[var(--accent)]"
                }`}
              />
            </dd>
