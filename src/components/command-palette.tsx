...
export function CommandPalette() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (gPending && !e.metaKey && !e.ctrlKey) {
        ...
        if (href) {
          e.preventDefault();
          if ((href === "/dashboard" || href === "/projects/new") && !signedIn) {
            router.push("/login");
          } else {
            router.push(href);
          }
  ...
  return (
    <>
      {gPending && (
        <div className="pointer-events-none fixed bottom-20 left-4 z-[55] rounded-lg border border-amber-900/50 bg-black/80 px-2 py-1 text-[10px] text-amber-300">
          G…
        </div>
      )}
      {open && (
        ...
        >
          <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-amber-900/40 bg-[#0a0a0a] shadow-2xl">
              ...
              {items.map((c, i) => (
                <li key={c.id}>
                  <button
                    ...
                    className={
                      i === active
                        ? "flex w-full items-center justify-between px-3 py-2.5 text-left text-sm bg-amber-500/15 text-white"
                        : "flex w-full items-center justify-between px-3 py-2.5 text-left text-sm text-stone-300 hover:bg-white/5"
                    }
            ...
            <div className="border-t border-white/10 px-3 py-2 text-[10px] text-stone-600">
              Esc close · ↑↓ navigate · Enter open · ? help · G then H/P/T/L/D/N
            </div>
          </div>
        </div>
      )}
