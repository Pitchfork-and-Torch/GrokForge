...
export { isNeonQuotaError } from "@/lib/neon-errors";
...
/** Lightweight liveness check for status + outage chrome. */
export async function probeDatabase(): Promise<DatabaseProbe> {
  try {
    await prisma.$queryRaw`SELECT 1`;
    return { ok: true, quota: false, message: null };
  } catch (error) {
    const message =
      error instanceof Error ? error.message.slice(0, 180) : "database unavailable";
    return { ok: false, quota: isNeonQuotaError(error), message };
  }
}
