...
export function loadConfig(): GrokForgeConfig {
  const apiBase = (process.env.GROKFORGE_API ?? DEFAULT_API_BASE).replace(/\/+$/, "");
  const token = process.env.GROKFORGE_TOKEN?.trim() || undefined;
  const defaultProject = process.env.GROKFORGE_DEFAULT_PROJECT?.trim() || undefined;
