...
export function registerWorkTools(server: McpServer): void {
  server.registerTool(
    "peek_work",
    {
      title: "Peek Work",
      description:
        "Peek the next ready OPEN leaf without claiming. GET /agent/work?projectSlug=. Uses args.projectSlug or GROKFORGE_DEFAULT_PROJECT.",
      inputSchema: {
        projectSlug: z
          .string()
          .optional()
    ...
    async ({ projectSlug }) => {
      try {
        ...
        const data = await apiRequest(
          "/agent/work",
          { query: { projectSlug: slug }, requireAuth: true },
          config,
        );
        return jsonResult(
  ...
  server.registerTool(
    "submit_work",
    {
      ...
      description:
        ...
        "Rejects bodies that look like xAI/model API keys. Never send SuperGrok keys.",
      inputSchema: {
        ...
        contentType: z
          .enum(["markdown", "json", "typescript", "javascript", "python", "txt"])
          .optional()
