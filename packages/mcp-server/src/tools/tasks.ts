...
import { apiRequest, errorResult, jsonResult, listTasksQuery, loadConfig } from "../client.js";
...
export function registerTaskTools(server: McpServer): void {
  server.registerTool(
    "list_open_leaves",
    {
      ...
      description:
        "List open (or filtered) GrokForge tasks / leaves. GET /tasks?status=OPEN&project=&limit=. Requires GROKFORGE_TOKEN (tasks:read).",
    ...
    async ({ status, project, limit }) => {
      try {
        ...
        const data = await apiRequest(
          "/tasks",
          {
            query: listTasksQuery({ status, project, limit }),
            requireAuth: true,
          },
          config,
        );
  ...
  server.registerTool(
    ...
    {
      title: "Get Task",
      description: "Fetch a single GrokForge task by id. GET /tasks/{taskId}. Requires tasks:read.",
      inputSchema: z.object({
